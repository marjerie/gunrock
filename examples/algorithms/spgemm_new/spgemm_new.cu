#include <gunrock/algorithms/spgemm_new.hxx>
...
void test_spmv(int num_arguments, char** argument_array) {
  if (num_arguments != 4) {
    std::cerr << "usage: ./bin/<program-name> a.mtx b.mtx n" << std::endl;
    exit(1);
  }
  ...
  // size_t num_of_elements_to_fill = 8353122973;
  // size_t num_of_elements_to_fill = 8353122973;
  ...
  // --
  // GPU Run
  float gpu_elapsed = gunrock::spgemm_new::run(A, B_csr, B, C);
